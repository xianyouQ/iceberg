...
public class BaseIncrementalTableScan
        extends BaseIncrementalScan<IncrementalTableScan, FileScanTask, CombinedScanTask>
        implements IncrementalTableScan {

    BaseIncrementalTableScan(Table table, Schema schema, TableScanContext context) {
        super(table, schema, context);
    }

    public BaseIncrementalTableScan(Table table) {
        this(table, table.schema(), TableScanContext.empty());
    }

    @Override
    protected BaseIncrementalTableScan newRefinedScan(
            Table newTable, Schema newSchema, TableScanContext newContext) {
        return new BaseIncrementalTableScan(newTable, newSchema, newContext);
    }
    ...
    private CloseableIterable<FileScanTask> appendFilesFromSnapshots(List<Snapshot> snapshots) {
        ...
        Set<ManifestFile> manifests =
                FluentIterable.from(snapshots)
                        .transformAndConcat(snapshot -> snapshot.allManifests(table().io()))
    ...
    private static List<Snapshot> appendsBetween(
        ...
        for (Snapshot snapshot :
            ...
            if (snapshot.operation().equals(DataOperations.REPLACE)) {
                //todo whether not allow contains replace snapshot between fromSnapshot and toSnapshot?
                continue;
            }
